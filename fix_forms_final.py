...
# Find the method with the docstring that needs to be fixed
method_pattern = r'def clean_violation_documents\(self\):.*?def clean_fine_amount\(self\):'
match = re.search(method_pattern, content, re.DOTALL)

if match:
    # Extract the matched method content
    old_method = match.group(0)
    
    # Get the position where the next method starts
    next_method_decl = 'def clean_fine_amount(self):'
    
    # Create the replacement (keep the next method declaration intact)
