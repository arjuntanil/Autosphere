...
from .models import CustomUser, Vehicle, Notice, PersonalizedVehicle, VehicleFine, UserNotification, VehicleModificationRequest, PasswordResetVerification, FinePayment
...
def rto_home(request):
    ...
    total_fines = VehicleFine.objects.count()
    unpaid_fines = VehicleFine.objects.filter(payment_status='Unpaid').count()
    
    # Get recent vehicles
    recent_vehicles = Vehicle.objects.all().order_by('-registered_at')[:5]
    
    # Get recent fines
    recent_fines = VehicleFine.objects.all().order_by('-imposed_date')[:5]
...
def impose_fine(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Normalize the registration number to handle different input formats
            normalized_reg = normalize_registration_number(reg_number)
            
            try:
                # Try both the normalized version and the original input
                vehicle = Vehicle.objects.filter(
                    Q(registration_number=normalized_reg) | 
                    Q(registration_number__iexact=reg_number)
                ).first()
                ...
                fine.imposed_date = timezone.now()
                fine.payment_status = 'Unpaid'
                fine.save()
                
                # Important: Save the form again to handle the file upload properly
                # When using commit=False, m2m fields and file fields need special handling
                form.save_m2m()  # This doesn't affect file fields, but is good practice
                
                # Ensure the uploaded file is properly saved
                if 'violation_document' in request.FILES:
                    ...
                    # Re-save the document to ensure it's properly stored
                    document = request.FILES['violation_document']
                    fine.violation_document = document
                    fine.save()
                    
                    # Debug info
                    doc_path = os.path.join(BASE_DIR, 'media', fine.violation_document.name)
                    print(f"PDF document saved to: {doc_path}")
                    print(f"File exists on disk: {os.path.exists(doc_path)}")
...
def rto_fine_list(request):
    ...
    # Initialize queryset
    fines = VehicleFine.objects.all().order_by('-imposed_date')
...
def fine_details(request, fine_id):
    try:
        ...
        return render(request, 'fine_details.html', {
            'fine': fine,
        })
...
def payment_receipt(request, receipt_number):
    ...
    try:
        ...
        context = {
            ...
            'razorpay_order_id': fine.razorpay_order_id,
            'razorpay_payment_id': fine.razorpay_payment_id,
            'razorpay_signature': fine.razorpay_signature,
        }
